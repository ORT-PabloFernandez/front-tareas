    ...
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="bg-zinc-900 rounded-xl border border-zinc-800 p-6">
          ...
          {role === "collaborator" && assignment.status === "pending" ? (
            <button className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg font-medium transition shadow">
              Iniciar Ejecución
            </button>
          ) : role === "collaborator" ? (
            <p className="text-zinc-400">Esta asignación ya no está pendiente. Revisá el listado de ejecuciones.</p>
          ) : (
            <p className="text-zinc-400">Modo lectura. Solo el colaborador asignado puede iniciar esta tarea.</p>
          )}
        </div>
      </div>
    </div>
